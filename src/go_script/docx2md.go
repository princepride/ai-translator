...
func docx2md(inputPath string, outputPath string, embed bool) error {
    ...
    // 将 Markdown 内容写入指定的输出文件
    err = ioutil.WriteFile(outputPath, buf.Bytes(), 0644)
    if err != nil {
        return err
    }

    fmt.Printf("转换成功: Markdown 文件已保存到 %s\n", outputPath)
    return nil
}
...
func main() {
    ...
    if flag.NArg() < 2 {
        fmt.Println("Usage: docx2md <input.docx> <output.md>")
        os.Exit(1)
    }
    ...
    if err := docx2md(inputPath, outputPath, embed); err != nil {
        log.Fatal(err)
    }
}